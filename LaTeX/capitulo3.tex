...
% conocer la intensidad implica conocer mR y nC en el tiempo
...
\vspace{10pt}
Where $\nu_{ph}$ is the lattice phonon vibration frequency and $K$ is the transition probability constant. Typically, one can expect $s \sim 10^{12} -10^{14}$ s$^{-1}$, which is consistent with the values of $\nu$ for most solids. The term $e^{\nicefrac{\Delta S}{k_B}}$ is a correction factor that accounts for the entropy change associated with the transition from the localized state to the conduction band. In a general practice, this term is often cosidered as temperature independent, but we will discuss this further in Chapter \ref{ch:4}.
