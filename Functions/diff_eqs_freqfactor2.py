...
def diff_eqs_freqfactor2(y0, t, value):
    """
    ...
    * jA_mn: recombination probability factor for recombination center j [cm3 s-1]
    ...
    ### where now alpha depends on each trap's experimental frequency factor.
    Args:
    ...
    nu_s = 2.53e+16
    K = 1
    alpha = value.kB * np.log(1 / (nu_s * K)) / np.log(300) 
    deltaS = alpha * np.log(temp_heating)
    #α=kB​⋅log(S /ν ​K​​)​/log(300​)
    
    # Computation of electron release probabilities
    p_I = freq_factor(np.log(value.S_I) * alpha * np.log(temp_heating)) * np.exp(-value.E_I / (value.kB * temp_heating))
    p_II = freq_factor(np.log(value.S_II) * alpha * np.log(temp_heating)) * np.exp(-value.E_II / (value.kB * temp_heating))
    p_III = freq_factor(np.log(value.S_III) * alpha * np.log(temp_heating)) * np.exp(-value.E_III / (value.kB * temp_heating))
    p_IV = freq_factor(np.log(value.S_IV) * alpha * np.log(temp_heating)) * np.exp(-value.E_IV / (value.kB * temp_heating))
    p_V = freq_factor(np.log(value.S_V) * alpha * np.log(temp_heating)) * np.exp(-value.E_V / (value.kB * temp_heating))
    p_s = freq_factor(np.log(value.S_s) * alpha * np.log(temp_heating)) * np.exp(-value.E_s / (value.kB * temp_heating))
    
    p_R_h = freq_factor(np.log(value.S_R_h) * alpha * np.log(temp_heating)) * np.exp(-value.E_R_h / (value.kB * temp_heating))
    p_NR_h = freq_factor(np.log(value.S_NR_h) * alpha * np.log(temp_heating)) * np.exp(-value.E_NR_h / (value.kB * temp_heating))
