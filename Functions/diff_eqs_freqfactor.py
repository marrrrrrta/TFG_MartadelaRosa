...
def diff_eqs_freqfactor(y0, t, value):
    ...
    * jA_mn: recombination probability factor for recombination center j [cm3 s-1]
    ...
    alpha = (1.5 * value.kB) / np.log(300)
    deltaS = alpha * np.log(temp_heating)
    
    # Computation of electron release probabilities
    p_I = freq_factor(deltaS) * np.exp(-value.E_I / (value.kB * temp_heating))
    p_II = freq_factor(deltaS) * np.exp(-value.E_II / (value.kB * temp_heating))
    p_III = freq_factor(deltaS) * np.exp(-value.E_III / (value.kB * temp_heating))
    p_IV = freq_factor(deltaS) * np.exp(-value.E_IV / (value.kB * temp_heating))
    p_V = freq_factor(deltaS) * np.exp(-value.E_V / (value.kB * temp_heating))
    p_s = freq_factor(deltaS) * np.exp(-value.E_s / (value.kB * temp_heating))
    
    p_R_h = freq_factor(deltaS) * np.exp(-value.E_R_h / (value.kB * temp_heating))
    p_NR_h = freq_factor(deltaS) * np.exp(-value.E_NR_h / (value.kB * temp_heating))
