...
def plot_column1(odeint_solution1, odeint_solution2, save_path, x_axis, legendloc, xaxis_label, fontsize=14):
    # Unpack the variables
    ...
    plt.tight_layout()
    plt.savefig(save_path + 'n_i evolution' + '.png', dpi=300, bbox_inches='tight')
    
def plot_column2(odeint_solution1, odeint_solution2, save_path, x_axis, value, legendloc, xaxis_label, fontsize=14):
    # Unpack the variables
    n_I1, n_II1, n_III1 ,n_IV1 ,n_V1 ,n_s1 ,m_R1 ,m_NR1 ,n_c1 , n_v1 = odeint_solution1.T
    n_I2, n_II2, n_III2 ,n_IV2 ,n_V2 ,n_s2 ,m_R2 ,m_NR2 ,n_c2 , n_v2 = odeint_solution2.T
    ...
    # Saving the results
    plt.savefig(save_path + 'Recombination' + '.png', dpi=300, bbox_inches='tight')
    plt.show()

def plot_column3(odeint_solution1, odeint_solution2, save_path, x_axis, value, xaxis_label, fontsize=14):
    # Unpack the variables
    n_I1, n_II1, n_III1 ,n_IV1 ,n_V1 ,n_s1 ,m_R1 ,m_NR1 ,n_c1 , n_v1 = odeint_solution1.T
    n_I2, n_II2, n_III2 ,n_IV2 ,n_V2 ,n_s2 ,m_R2 ,m_NR2 ,n_c2 , n_v2 = odeint_solution2.T
    ...
    denominator[denominator == 0] = np.nan
    plt.plot(x_axis, (n_c1 + n_I1 + n_II1 + n_III1 + n_IV1 + n_V1 + n_s1)/denominator)
    plt.xlabel(xaxis_label, fontsize=14)
    ...
    denominator[denominator == 0] = np.nan
    plt.plot(x_axis, (n_c2 + n_I2 + n_II2 + n_III2 + n_IV2 + n_V2 + n_s2)/denominator)
    plt.xlabel(xaxis_label, fontsize=14)
    plt.ylabel('Charge neutrality ratio', fontsize=14)
    plt.title('Temperature independent model', fontsize=16)
    
    plt.suptitle('Charge neutrality', fontsize=16)
    
    # Saving the results
    plt.savefig(save_path + 'Charge neutrality' + '.png', dpi=300, bbox_inches='tight')
    plt.show()
